...
def parse_jma_xml(xml_bytes: bytes) -> List[Alert]:
    ...
    issued_str = _text(root, "//Head/ReportDateTime/text()") or _text(root, "//Report/Head/ReportDateTime/text()")

    issued_at = (
        datetime.fromisoformat(issued_str.replace("Z", "+00:00")).astimezone(timezone.utc)
        if issued_str
    ...
    for area_node in root.xpath("//Body//Warning//Item | //Body//Area//Item | //Report/Body//Item"):
        area_name = _text(area_node, "./Area/Name/text()") or _text(area_node, "./Area/text()") or "Unknown"
        category = _text(area_node, "./Kind/Name/text()") or _text(area_node, "./Kind/text()") or "Unknown"
        severity = _text(area_node, "./Kind/Status/text()") or _text(area_node, "./Status/text()") or "Unknown"
        ...
        alert_id = sha256("|".join([primitive["area"], primitive["category"], primitive["severity"], primitive["issued_at"]]).encode("utf-8")).hexdigest()[:16]
